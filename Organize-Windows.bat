...
if not exist "%DOWNLOADS_DIRECTORY%\Executables\" mkdir "%DOWNLOADS_DIRECTORY%\Executables"
...
REM 4. Si el archivo es un archivo regular, el script utiliza los modificadores de variables del lote para extraer la extensión del archivo y guardarla en la variable "extension".

REM 5. El script luego usa un caso de instrucción para verificar la extensión del archivo y moverlo al directorio correspondiente según su extensión. Si la extensión no coincide con ninguno de los casos definidos, se mueve al directorio Others.
...
for %%F in ("%DOWNLOADS_DIRECTORY%\*.*") do (
    if exist "%%F" (
        ...
        if /i "!extension!"=="jpg" (
            ...
            move /-Y "%%F" "%DOWNLOADS_DIRECTORY%\Executables\%%~nxF"
        ) else (
